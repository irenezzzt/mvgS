# perfect theoretical null make sure SKATO is fine
...
t1e.sim<-function(xx){
  ...
  return(iSKAT(Z=Z,Y=matrix(Y,ncol=1),E=matrix(E,ncol=1),missing_cutoff = 0.9, MAF_cutoff = 0.5)$pvalue)
  #return(c(p1,p2,p3,p4,p5,p6))
}

s<-Sys.time()
t1e.res<-matrix(unlist(lapply(X=1:1000, FUN=t1e.sim)),ncol=6,byrow=T)
time.cost<-Sys.time()-s

iskat.t1e.theoretical.null<-lapply(X=1:1000, FUN=t1e.sim)
...
write.csv(t1e.res, "~/Desktop/MgS/theoretical_null_t1e.csv")
...
for (xx in 1:1000){
  ...
  Y<-rnorm(n, 0.002, 0.073)
  # delete all indiv=0 SNPs
  if(sum(colMeans(Z)!=0)==1){
    pv=NA}else{
      Z<-Z[,colMeans(Z)!=0]
      pv<-iSKAT(Z=Z,Y=matrix(Y,ncol=1),E=matrix(E,ncol=1),missing_cutoff = 0.9, MAF_cutoff = 0.5)$pvalue
      }
  temp<-c(temp, pv) 
}
mean(temp<0.05,na.rm = T) # ] 0.07818411
...
for (xx in 1:1000){
  ...
  if(sum(colMeans(Z)!=0)==1){pv=NA}else{
        Z<-Z[,colMeans(Z)!=0]
        if(sum(colMeans(Z)<0.05)==0){pv=NA}else{
        pv<-iSKAT(Z=Z,Y=matrix(Y,ncol=1),E=matrix(E,ncol=1),missing_cutoff = 0.9, MAF_cutoff = 0.05)$pvalue}}
  temp<-c(temp, pv) 
}
mean(temp<0.05,na.rm = T)
